...
detectRUNS is a R package for detecting **Runs of Homozygosity** (ROH/ROHom)
...
- `detectRUNS_technical_report` — a technical report with validation details
...
`build_vignettes = TRUE` (see [Building the vignette](#building-the-vignette)
below).
...
`devtools::build_vignettes()` is deprecated. To render the vignette during
development:
...
To build the package tarball with the compiled vignette included (e.g. before
a release):
