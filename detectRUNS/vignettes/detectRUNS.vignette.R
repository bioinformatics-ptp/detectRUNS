...
tableRuns(slidingRuns,genotypeFile = genotypeFilePath, mapFile = mapFilePath, threshold = 0.7)

## ---- echo=FALSE, message=FALSE, cache=FALSE-----------------------------
head(Froh_inbreeding(runs = slidingRuns,mapFile = mapFilePath,genome_wide = TRUE))

## ---- echo=FALSE, message=FALSE, cache=FALSE, fig.width=5, fig.height=4----
plot_InbreedingChr(runs = slidingRuns, mapFile = mapFilePath, style = "FrohBoxPlot")

## ---- message=FALSE------------------------------------------------------
savedRunFile <- system.file("extdata", "Kijas2016_Sheep_subset.sliding.csv", package="detectRUNS")
