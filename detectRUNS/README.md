...
- `detectRUNS.vignette` — a full tutorial covering the main workflow
- `detectRUNS_technical_report` — a technical report with validation
  details

List the available vignettes:

``` r
vignette(package = "detectRUNS")
```
...
vignette("detectRUNS_technical_report", package = "detectRUNS")
```

Note: vignettes are only available if the package was installed with
`build_vignettes = TRUE` (see [Building the
vignette](#building-the-vignette) below).
...
`devtools::build_vignettes()` is deprecated. To render the vignette
during development:

``` r
rmarkdown::render("detectRUNS/vignettes/detectRUNS.vignette.Rmd")
```

To build the package tarball with the compiled vignette included
(e.g. before a release):

``` r
devtools::build("detectRUNS", vignettes = TRUE)
```
