...
reportRUNS <- function(
  ...
  # Handles both base-graphics functions and functions returning ggplot objects.
  # --------------------------------------------------------------------------
  .save_plot <- function(tag, fn, ...) {
    ...
    ok   <- tryCatch({
      ...
      result <- tryCatch({
        val <- fn(...)
        # ggplot objects must be printed explicitly to the open device
        if (inherits(val, "gg") || inherits(val, "ggplot"))
          print(val)
        TRUE
      }, error = function(e) {
    ...
    file.path(paste0(prefix, "_plots"), paste0(tag, ".png"))
  }
  ...
  if (format == "html") {
    ...
    # Inline all PNG images as base64 data URIs so the HTML is self-contained
    # and opens correctly from any location (file://, browser, email, etc.)
    html_lines <- .inline_images_base64(html_lines, output_dir)
...
# ---------------------------------------------------------------------------
# Internal: base64 image inlining
# ---------------------------------------------------------------------------

#' Encode a PNG file as a base64 data URI (pure R, no extra packages)
#' @keywords internal
.b64_encode_file <- function(path) {
  raw_vec <- readBin(path, "raw", n = file.info(path)$size)
  if (length(raw_vec) == 0L) return("")
  alpha <- strsplit(
    "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", "")[[1L]]
  n     <- length(raw_vec)
  n_pad <- (3L - n %% 3L) %% 3L
  raw_vec <- c(raw_vec, raw(n_pad))
  mat <- matrix(as.integer(raw_vec), nrow = 3L)
  chars <- as.vector(rbind(
    alpha[bitwShiftR(mat[1L, ], 2L) + 1L],
    alpha[bitwOr(bitwShiftL(bitwAnd(mat[1L, ], 3L), 4L), bitwShiftR(mat[2L, ], 4L)) + 1L],
    alpha[bitwOr(bitwShiftL(bitwAnd(mat[2L, ], 15L), 2L), bitwShiftR(mat[3L, ], 6L)) + 1L],
    alpha[bitwAnd(mat[3L, ], 63L) + 1L]
  ))
  if (n_pad >= 2L) chars[length(chars) - 1L] <- "="
  if (n_pad >= 1L) chars[length(chars)]       <- "="
  paste(chars, collapse = "")
}


#' Replace relative PNG src attributes with inline base64 data URIs
#' @keywords internal
.inline_images_base64 <- function(html_lines, base_dir) {
  pat <- 'src="([^"]+\\.png)"'
  vapply(html_lines, function(line) {
    if (!grepl(pat, line, perl = TRUE)) return(line)
    rel <- regmatches(line, regexpr('(?<=src=")[^"]+\\.png', line, perl = TRUE))
    if (length(rel) == 0L) return(line)
    abs_path <- file.path(base_dir, rel)
    if (!file.exists(abs_path)) return(line)
    b64  <- .b64_encode_file(abs_path)
    sub(pat, sprintf('src="data:image/png;base64,%s"', b64), line, perl = TRUE)
  }, character(1L))
}
