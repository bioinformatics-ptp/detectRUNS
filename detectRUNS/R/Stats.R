...
runs_summary <- function(runs,mapFile,Class=2, snpInRuns=FALSE,genotype_path){
  ...
  summary_ROH_mean_chr = dcast(summary_ROH_mean_chr1,CHROMOSOME ~ GROUP ,value.var = "sum")
  summary_ROH_mean_chr
  
  #RESULTS!!!!!
  summary_ROH_count = as.data.frame( daply(runs,.(CLASS,GROUP),nrow))
  summary_ROH_percentage = t(as.data.frame( t(summary_ROH_count)/colSums(summary_ROH_count)))
  
  #RESULTS!!!!!
  summary_ROH_count_chr = as.data.frame( daply(runs,.(CHROMOSOME,GROUP),nrow))
  summary_ROH_percentage_chr = t(as.data.frame( t(summary_ROH_count_chr)/colSums(summary_ROH_count_chr)))
